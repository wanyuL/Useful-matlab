...
linestyle = 'k-';
subplot(2,2,1); 
%plot the number of mRNA molecules for one sample trajectory of the stochastic system
plot(tt/60,results(:,2),linestyle);           
xlabel('time(min)'); ylabel('mRNA (#)');
axis([0,tmax/60,0,Rmax]);
title('Time dependent dynamics');
hold on; 
%plot the mean number of mRNA as a function of time
plot(tt/60,(kR/gR)*(1-exp(-gR*tt)),'r');
hold off;

subplot(2,2,3); 
%plot the number of proteins for one sample trajectory of the stochastic system
plot(tt/60,results(:,3),linestyle);
xlabel('time(min)'); ylabel('protein (#)');
axis([0,tmax/60,0,Pmax]);
title('Time dependent dynamics');
hold on; 
%plot the mean number of protein as a function of time
...
ndrop=3600*3; %number of time points to drop to ensure system reached steady state 
